...
## Set numpy and torch random keys
np.random.seed(SEED)
...
if train:
    sched_node = optax.exponential_decay(init_value=init_lr, transition_steps=transition_steps, decay_rate=0.99)
    ...
    for epoch in range(nb_epochs):
        ...
        for i, batch in enumerate(train_loader):
            model, opt_state_node, loss, (min_pixel, max_pixel) = train_step(model, batch, opt_state_node, train_key)
            loss_epoch += loss

        loss = loss_epoch / (i+1)
        # model, opt_state_node, loss, (min_pixel, max_pixel) = train_step(model, (train_data, train_data_tgt), opt_state_node, train_key)
...
ax = sbplot(np.array(losses_node), x_label='Epoch', y_label='L2', y_scale="linear", label='Losses Node', ax=ax, dark_background=False);
plt.legend()
plt.draw();
